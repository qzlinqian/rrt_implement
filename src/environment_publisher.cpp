...
//  ROS_INFO("ready to subscribe world info");
...
void Environment_publisher::plot() {
  /*while*/ if (ros::ok()){ //should not use "while" here, or the world will plot again and again
    ...
    sleep(1);
    for(int i=0; i<collision_object_points.size(); i++){      //collision: blue
...
//  std::cout<<object.name<<" points published"<<std::endl;
  sleep(1);
  publisher.publish(marker_lines);
//  std::cout<<object.name<<" lines published"<<std::endl;
}
